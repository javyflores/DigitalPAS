...
class SecvisitasController extends Controller
{
    public function index(Request $request)
    {
		...
		$consult = Visitas::where('edo', $sec)->get();
        ...
        if ($consult) {
	        ...
	        return view('seccional.secvisitas', [
	            'codigo' => $codigo,
	            'usuario' => $usuario,
	            'tipouser' => $tipouser,
	            'rol' => $rol,
	    	...
	    	return redirect('/dirsec')->with('error', 'No posees solicitudes activas');
	    }
    }
    ...
    public function consulta(Request $request){
        ...
        if (isset($_POST['req_seg'])){
	    	...
	    	$sol_tra = Sol_tramite::find($req);

	    	$requerimiento = Req_seguimiento::where('req_seg', $req_seg)->get();


            return view('seccional.secvisitcons', [
                'codigo' => $codigo,
                'usuario' => $usuario,
	            'tipouser' => $tipouser,
	            'rol' => $rol,
